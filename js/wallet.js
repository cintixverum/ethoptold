function depositToken() {
  ...
  contractFunctions.depositToken(depositTokenAddress,value);
}  
...
$("#body > div.wallet-container > div > div > form > input").on('click', function() {
  ...
  if(depositWithdraw === "deposit") {
    ...
    let depositTokenAddress = $(depositTokenQuery).attr("address");
    let value = parseFloat($('#body > div.wallet-container > div > div > form > div:nth-child(2) > div > input').val());
    depositTokenAddress === "0x0000000000000000000000000000000000000000" ? contractFunctions.depositETH(value) : tokenFunctions.approveToken(contractAddress,value,depositToken); 
  } 
});
...
$("#body > div.wallet-container > div > div > form > input").on('click', function() {
  ...
  if(depositWithdraw === "withdraw") {  
    ...
    let depositTokenAddress = $(depositTokenQuery).attr("address");
    let value = parseFloat($('#body > div.wallet-container > div > div > form > div:nth-child(2) > div > input').val());  
    depositTokenAddress === "0x0000000000000000000000000000000000000000" ? contractFunctions.withdrawETH(value) : contractFunctions.withdrawToken(depositTokenAddress,value);  
  }  
});
